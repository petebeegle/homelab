...
| `immich` | Automated | Branch overlay deploys the production app shape into `immich-${branch_slug}`, with reduced dev PVC/resource sizes and an in-cluster API ping. It requires development base reconciliation for `local-path-provisioner`, `cloudnative-pg`, `nfs-csi`, and `gateway`. |
