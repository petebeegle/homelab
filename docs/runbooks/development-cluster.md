...
The development base intentionally includes CRDs, Cilium, cert-manager/certs, Gateway API, NFS CSI, whoami, and a development-only Authentik overlay for Jellyfin SSO validation. Games/media apps, Cloudflare tunnels, Renovate, VPN, and the full monitoring stack are omitted to keep the single-node cluster resource-conscious and to avoid production-only secrets or traffic paths unless a test explicitly needs them.
