...
Pushes to `origin` and automatic PR creation require verifier approval for the exact `HEAD` in `.codex/tmp/verifier-approved` plus a valid `.codex/tmp/verifier-attestation.yaml` with `approved_head` equal to that same SHA. The verifier `agent_id` must be a concrete identity and must differ from the implementation owner `agent_id`.
...
- Deterministic local attestations make the implementation owner and verifier roles machine-checkable without relying on conversational context.
...
- Generic identities such as `codex`, `assistant`, `planner`, `parent`, `main`, `self`, and `orchestrator` are not valid implementation owner or verifier identities.
