...
1. Run `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` from repo root and parse FEATURE_DIR and AVAILABLE_DOCS list. All paths must be absolute. For arguments containing single quotes, prefer double quotes when possible; otherwise use shell-safe single-quote escaping.
