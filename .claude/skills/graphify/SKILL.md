...
Next steps:
  /graphify query "<question>"        - query the graph (BFS/DFS traversal)
  ...
  /graphify --obsidian                - export Obsidian vault
```
...
**Wiki auto-regeneration:** After Steps 4–8 complete, check if `graphify-out/wiki/` exists. If it does, automatically re-run Step 6b to regenerate the wiki against the updated graph — no flag needed, the presence of the directory is the signal.

```bash
if [ -d graphify-out/wiki ]; then
  $(cat graphify-out/.graphify_python) -c "
import json
from graphify.build import build_from_json
from graphify.wiki import to_wiki
...
extraction = json.loads(Path('graphify-out/.graphify_extract.json').read_text()) if Path('graphify-out/.graphify_extract.json').exists() else json.loads(Path('graphify-out/graph.json').read_text())
analysis   = json.loads(Path('graphify-out/.graphify_analysis.json').read_text())
labels_raw = json.loads(Path('graphify-out/.graphify_labels.json').read_text()) if Path('graphify-out/.graphify_labels.json').exists() else {}
...
n = to_wiki(G, communities, 'graphify-out/wiki', community_labels=labels or None, cohesion=cohesion, god_nodes_data=gods)
print(f'Wiki regenerated: {n} articles updated')
"
fi
```
...
**Wiki-first:** If `graphify-out/wiki/index.md` exists, read it first to find which community article is most relevant to the question, then read that article for rich context before running the graph traversal below. Wiki articles contain pre-synthesized community summaries with cohesion scores and cross-links — prefer them over raw graph traversal for questions about architecture, communities, or high-level relationships. Fall back to graph traversal when the wiki doesn't cover the question.
