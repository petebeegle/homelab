...
from .flux import delete_branch_added_base_kustomizations, reconcile_flux, restore_flux_system_source, verify_cluster_base
    ...
    *,
    ...
    try:
        ...
        if config.include_cluster_base:
            base_cleanup = verify_cluster_base(config, runner=runner, defer_cleanup=True, defer_restore=True)
        ...
        if base_cleanup is not None:
            try:
                ...
                if failure is None:
                    ...
                    print(f"cluster base restore failed after primary error: {cleanup_exc}", file=sys.stderr)
            if not config.keep:
                try:
