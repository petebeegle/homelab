...
- The main agent may plan, inspect, stage required ignored local config, and coordinate implementation work, but tracked implementation and verifier work must be delegated to separate implementation and verifier subagents whenever runtime tooling exposes them.
- The main agent may perform tracked implementation or verifier work only when subagent tooling is unavailable or higher-priority runtime policy blocks delegation and the user explicitly consents for that task. Record the approved fallback in `.codex/tmp/pr-summary.md`.
...
- Before changing tracked files, implementation agents must record `.codex/tmp/implementation-owner-attestation.yaml` with matching delegation token evidence under `.codex/tmp/delegation-tokens/`; `tools/codex-harness/validate_workflow_attestations.py` is the enforcement source for owner and verifier attestation validity.
...
- After verifier sign-off for the exact `HEAD`, record `.codex/tmp/verifier-approved` and `.codex/tmp/verifier-attestation.yaml` with separate verifier delegation token evidence; the verifier `agent_id`, `delegation_token`, and `delegation_token_path` must differ from the implementation owner evidence. Then create the PR without intervention and delete the sibling clone only after PR creation succeeds.
